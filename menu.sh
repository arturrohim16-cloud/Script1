...
# IP & ISP (Dibatasi karakternya agar tidak merusak baris)
IP_VPS=$(curl -s ifconfig.me)
ISP=$(curl -s ipinfo.io/org | cut -d " " -f 2-10 | head -c 15)
OS_VER=$(lsb_release -ds | head -c 15)

# Traffic Usage (Membutuhkan vnstat)
...
# --- INFO SISTEM (VERSI RAPI TANPA PRINTF) ---
...
echo -e "  ${L_CYAN}TODAY   :${NC} ${GREEN}$TODAY_USAGE${NC}              ${L_CYAN}MONTH  :${NC} ${BG_RED} $MONTH_USAGE ${NC}"
echo -e "${L_PURPLE}  ---------------------------------------------------------${NC}"

# --- SERVICE STATUS ---
echo -e "            ${L_CYAN}[ SERVICE STATUS - GOOD ]${NC}"
echo -e "${L_PURPLE}  ---------------------------------------------------------${NC}"
echo -e "  ${L_CYAN}PROXY :${NC} $(check_service squid)  ${L_CYAN}SSH :${NC} $(check_service ssh)  ${L_CYAN}NGINX :${NC} $(check_service nginx)"
...
case $opt in
    ...
    [rR]) reboot ;;
    [xX]) exit 0 ;;
    *) echo "Pilihan salah!" && sleep 1 && menu ;;
esac
